...
- **Black** — undoes the most-recently-**resolved Red or Blue** on its slot, *at the
  moment Black itself resolves* (resolution order, NOT placement order). Order-agnostic:
  forward it counters the stone placed just before it; reverse it counters the one
  placed just after. Cannot itself be undone — and **never pulls a White**: a White
  lock is untouchable (as everywhere in Stonelock), so a locked slot fizzles Black
  entirely, shielding any Red beneath it too.

...
const CHARMS = {
  ...
  spite:        { label: 'Spite Engine',      blurb: 'When you lose a hand, your board reads +1 the next hand.', on: { handLost: g => { g.spitePending = true; }, handStart: g => { if (g.spitePending) { g.handBuff = (g.handBuff || 0) + 1; g.spitePending = false; } } } },
  momentum:     { label: 'Momentum',          blurb: 'When you win a hand, your board reads +1 the next hand. Stacks up to 3 times; a loss resets it.', on: { handWon: g => { g.winStreak = (g.winStreak || 0) + 1; }, handLost: g => { g.winStreak = 0; }, handStart: g => { if (g.winStreak) g.handBuff = (g.handBuff || 0) + Math.min(3, g.winStreak); } } },
  counterpunch: { label: 'Counterpunch',      blurb: 'When you lose your first hand each table, recover 3 Standing.', on: { handLost: g => { if (!g.cpDone) { g.cpDone = true; g.standing = Math.min(g.maxStanding, g.standing + 3); } } } },
  tithe:        { label: 'Tithe',             blurb: 'When you win a hand by 4+, press 1 extra Standing.' },
  ...
  cycle:        { label: 'Cycle',             blurb: 'Each hand, discard 1 card and draw 1.', cycleEach: 1 },
  ...
  reckless:     { label: 'Reckless Wager',    blurb: 'Your board reads +1 every hand. You lose 1 more Standing from every lost hand.', dmgReduce: -1, on: { handStart: (g) => { g.handBuff = (g.handBuff || 0) + 1; } } },
  ...
  motherlode:   { label: 'Motherlode',         blurb: 'Each Red phantom you field scores +1.', bossOnly: 1, persona: 'The Miner', phantomValue: 1 },
  ...
  highwayman:   { label: "Highwayman's Cut",    blurb: 'When you steal a card with Blue, it reads +2 on your board.', bossOnly: 1, persona: 'The Wagoner', cardBonus: (c) => (c.owner === 0 && c.origOwner != null && c.origOwner !== c.owner) ? 2 : 0 },
  followingsea: { label: 'Following Sea',       blurb: 'When you win a hand, your whole board reads +1 the next hand.', bossOnly: 1, persona: 'The Deckhand', on: { handWon: (g) => { g.pressNext = true; }, handStart: (g) => { if (g.pressNext) { g.handBuff = (g.handBuff || 0) + 1; g.pressNext = false; } } } },
  secondwind:   { label: 'Second Wind',         blurb: 'When your Standing would first break each act, you survive at 1.', bossOnly: 1, persona: 'The Old Hand' },
...
const CHARM_GLYPH_OFF = {
  ...
  '◉': [0.003, -0.072], '⇄': [0.003, -0.022], '✦': [0.003, 0.031], '✷': [0.003, 0.028],
  '♦': [0.003, -0.041], '▣': [0.006, -0.075], '⊚': [0.003, -0.022], '♛': [0.003, 0.031], '≈': [0.003, 0.003],
  '⧉': [-0.012, -0.055], // matchedset: diagonal twin-square reads low-right of its bbox, nudge up-left
...
if (typeof window !== 'undefined') {
  ...
  module.exports = {
    ...
    fixerDeal, charmCat, charmWorth, CHARM_CATS, CHARM_META,

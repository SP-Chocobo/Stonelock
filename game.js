...
const CHARMS = {
  ...
  tempering:    { label: 'Tempering',          blurb: 'Enter an Elite or Boss at 80%+ Standing and your maximum Standing grows +1 (up to +4 a run).', on: { fightStart: g => { const n = g.curNode; if (n && (n.type === 'elite' || n.type === 'boss') && g.standing >= g.maxStanding * 0.8 && (g.temperCount || 0) < 4) { g.temperCount = (g.temperCount || 0) + 1; g.maxStanding += 1; g.standing += 1; log('Tempering — you meet the hard fight strong; your maximum Standing grows.', 'you'); updateCircuitHud(); } } } },
  hardened:     { label: 'Hardened',          blurb: 'Your maximum Standing is +3.', maxStandingAdd: 3 },
  fieldsurgeon: { label: 'Field Surgeon',     blurb: 'Enter a fight below 50% Standing and you heal 6.', on: { fightStart: g => { if (g.standing < g.maxStanding * 0.5) { g.standing = Math.min(g.maxStanding, g.standing + 6); log('Field Surgeon — patched up before the bout; you heal 6 Standing.', 'you'); updateCircuitHud(); } } } },
  ...
  counterpunch: { label: 'Counterpunch',      blurb: 'The first hand they take from you each table, heal 3.', on: { handLost: g => { if (!g.cpDone) { g.cpDone = true; g.standing = Math.min(g.maxStanding, g.standing + 3); } } } },
  ...
  resonance:    { label: 'Resonance',         blurb: 'Every third stone you place at a table, recover 2 Standing.',
